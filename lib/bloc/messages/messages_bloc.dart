...
class MessagesBloc extends Bloc<MessagesEvent, MessagesState> {
  final FirebaseFirestore _db = FirebaseFirestore.instance;
  final FirebaseAuth _auth = FirebaseAuth.instance;
  StreamSubscription? _chatsSubscription; // Non-late to avoid initialization errors
        ...
        _chatsSubscription = _db
          ...
          for (var chatDoc in chatsSnapshot.docs) {
            ...
            _db.collection('doctors').doc(doctorId).get().then((doc) {
              ...
              _updateConversations(conversations, chatId, doctorId, doctorName, participantData);
            }).catchError((e) {
  ...
  void _updateConversations(List<Conversation> conversations, String chatId, String doctorId, String doctorName, Map<String, dynamic> participantData) {
    _db.collection('chats').doc(chatId).collection('messages')
      ...
      print('MessagesBloc: Loaded ${conversations.length} conversations');
      emit(MessagesLoadedState(List.from(conversations))); // Emit new list to trigger UI update
    }).catchError((e) {
      print('Error fetching last message: $e');
      emit(MessagesErrorState('Failed to load messages: $e'));
    });
